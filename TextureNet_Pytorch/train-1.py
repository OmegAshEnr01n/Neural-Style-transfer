...
def train(loader, target_texture, device, num_epochs, batch_sz, learning_rate, image_sz, max_iter):
	...
	loss_layers, weights, loss_fns, style_layers, content_layers = get_losses(device)
	...
	for i in range(num_epochs):
		...
		for i, data in enumerate(loader):
			...
			content_targets = [A.detach() for A in descriptor_net(target_texture, content_layers)]
...
def main():
	...
	loader, target=create_data(args.data_dir, args.texture, device,  args.batch_size)
	train(loader, target, device, args.num_epoch, args.batch_size,learning_rate, image_sz, args.max_iter)
